...
public class TC_LCO_Client_Context_ArgyleAtomicFI extends BaseClass {
    ...
    public Object[][] getData() throws IOException {
        ...
        int totalrows=excel.getS3RowCount("LCODataArgyleAtomicfi");
        int totalcols=excel.getS3CellCount("LCODataArgyleAtomicfi",1);
        String LoginData[][]=new String[totalrows][totalcols];
        for(int i=1;i<=totalrows;i++) //1
        {
            for(int j=0;j<totalcols;j++) //0
            {
                LoginData[i-1][j]=excel.getS3CellData("LCODataArgyleAtomicfi", i, j);
            }
        }
        return LoginData;
    }
    ...
    public void lcoClient(String employer) throws InterruptedException {
        ...
        for(String child: allWindow)
        {
            if(!parent.equalsIgnoreCase(child))
            {
                ...
                Identity_VerificationPage.isSupported();
                logger.info("Entered employee is supported for LCO");
