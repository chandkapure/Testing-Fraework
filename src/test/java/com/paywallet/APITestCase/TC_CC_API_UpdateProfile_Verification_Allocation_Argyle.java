...
public class TC_CC_API_UpdateProfile_Verification_Allocation_Argyle extends BaseClass {

    String emailid =  "TC_CC_API_UpdateProfile_Verification_Verification_AtomicFI"+ randomNum +"@2bgcse6d.mailosaur.net";
    ...
    public void UpdateProfileAPI(String provider, String context, String firstName, String middleName, String lastName,
        ...
        System.out.println("TC_CC_API_UpdateProfile_Verification_Allocation_Argyle email id is "+emailid);
        System.out.println("TC_CC_API_UpdateProfile_Verification_Allocation_Argyle phone number is "+mobile);
//  Employer Type Ahead
...
//  SDK Login Starts
        ...
        getDriver().get(OkinusAPISkipSDKLoginArgyle);
...
//    UpdateProfile API Flow
        String profiless = "DEPOSIT_ALLOCATION";
...
//   SDK Login Flow
        ...
        UtilityClass.ClientOTP();
        CC_SDKLoginPage_Client skippayroleLoginn = new CC_SDKLoginPage_Client(getDriver());
        skippayroleLoginn.DownloadExcelData();
        skippayroleLoginn.clickAllocationConform();
        skippayroleLoginn.clickUpdateDirectDeposit();
        skippayroleLoginn.setConfirm();
        CC_Customer_ContextFlow customercontext = new CC_Customer_ContextFlow(getDriver());
        customercontext.setVerificationcode(code);
        Thread.sleep(2000);
        customercontext.setVerificationcontinue();
        skippayroleLoginn.verifySDKSuccessScreen();
        logger.info("SDK Success screen displayed successfully");
...
//  Fetch Request ID
        AllocationFetchRequestID(RequestID);
    ...
    private void AllocationFetchRequestID(String requestID) {
        RequestSpecification FetchrequestID = RestAssured.given().header("Authorization", CodeConvergenceToken).header("X-request-Id", requestID);
        Response fetchresponse = FetchrequestID.accept("application/json").get(APIURL+"/api/v1/request");
        String FetchReqBody = fetchresponse.getBody().asString();
        logger.info("Fetch Request ID Request Sent");
